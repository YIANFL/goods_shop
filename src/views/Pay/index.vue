...
export default {
  ...
  methods: {
    ...
    async open () {
      ...
      // 需要支付成功或者是失败,成功了进行路由跳转,支付失败展示提示信息
      // 要发送请求,获取后台的数据
      // 长轮询
      // 判断有没有定时器,没有的话设置一个定时器
      if (!this.timer) {
        this.timer = setInterval(async () => {
          // 发请求获取用户支付的状态
          const result = await this.$API.reqPayState(this.orderNum)
          if (result.code === 20) {
            // 支付成功之后,清除定时器,隐藏二维码
            clearInterval(this.timer)
            this.timer = null
            // 保存支付成功之后的code值,用于判断是不是支付了
            this.payStateNum = result.code
            // 关闭弹出框
            this.$msgbox.close()
            alert(111)
            // 跳转到下一页路由
